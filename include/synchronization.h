...
typedef struct {
    pthread_mutex_t intersection_lock;   // Core intersection access control
    pthread_cond_t condition_vars[4];    // Signals for each lane
    int current_lane;                    // Lane currently occupying intersection
    pthread_t lock_holder;               // Thread holding the intersection lock
    time_t lock_acquisition_time;        // When current lane got access
    bool intersection_available;         // Is intersection free for allocation
    int active_quadrants;                // Currently occupied intersection quadrants
} IntersectionMutex;

// Intersection lifecycle functions
...
// Priority inversion handling functions
...
// Resource allocation state functions
